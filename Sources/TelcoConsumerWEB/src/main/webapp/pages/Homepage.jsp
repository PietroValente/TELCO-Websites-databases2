...
<script type="text/javascript">
  ...
  function rejectedpurchases() {
	  <%if (request.getParameter("solvent").compareTo("false") == 0) {%>
	  	document.getElementById('rejectedOrder').style.display = 'block';
	  <%}else{%>
	  	document.getElementById('rejectedOrder').style.display = 'none';
	  <%}%>
  }
...
<body>
    ...
    <div class="container d-flex align-items-center justify-content-between">
      ...
      <nav id="navbar" class="navbar">
        <ul>
          ...
          <li><a class="nav-link" href="<%= request.getContextPath() %>/BuyService.do">Buy Service</a></li>
  ...
  <main style="padding:50px" id="main">
	...
	<br>
    <div id="rejectedOrder">
      <h1 style="text-align: center;">Rejected orders:<br></h1>
      <br>
      <div id="rejectedTable" class="row row-cols-1 row-cols-md-3 g-4"></div>
      <br>
      <hr>
      <br>
    </div>
    ...
    var command = '';
        ...
        command +=    '</ul>'
          + '<p style="padding:5px;text-align: center;">From <b>' + "<%out.print(request.getParameter("PackageBestPrice" + i));%>" + '$</b> montly</p></p>'
            ...
            container = document.getElementById('rejectedTable');
            var command = '';
            <%for (int j = 0; j < Integer.parseInt(request.getParameter("allRPackageSize")); j += 1) {%>
    		<% String i = ("" + j).trim();%>
            command += '<div class="col">'
	                ...
	                +'<table style="margin-left:auto;margin-right:auto;">'
	                  +'<tr>'
	                    +'<td style="padding: 0 15px;">'
	                      +'<b>Package: </b>'
	                    +'</td>'
	                    +'<td style="padding: 0 15px;">' + "<%out.print(request.getParameter("PackageRName" + i));%>" + '</td>'
	                  +'</tr>'
	                  +'<tr>'
	                    +'<td style="padding: 0 15px;padding-top:5px;">'
	                      +'<b>Order date:</b>'
	                    +'</td>'
	                    +'<td style="padding: 0 15px;padding-top:5px;">' + "<%out.print(request.getParameter("PackageRTimestamp" + i));%>" + '</td>'
	                  +'</tr>'
	                  +'<tr>'
	                    +'<td style="padding: 0 15px;padding-top:5px;"><b>Starting period:</b></td>'
	                    +'<td style="padding: 0 15px;padding-top:5px;">' + "<%out.print(request.getParameter("PackageRSP" + i));%>" + '</td>'
	                  +'</tr>'
	                  +'<tr>'
	                    +'<td style="padding: 0 15px;padding-top:5px;"><b>End period:</b></td>'
	                    +'<td style="padding: 0 15px;padding-top:5px;">' + "<%out.print(request.getParameter("PackageREP" + i));%>" + '</td>'
	                  +'</tr>'
	                  +'<tr>'
	                    +'<td style="padding: 0 15px; padding-top:5px;"><b>Refused times:</b></td>'
	                    +'<td style="padding: 0 15px; padding-top:5px;">' + "<%out.print(request.getParameter("PackageRefusedTimes" + i));%>" + '</td>'
	                  +'<tr>'
	                    +'<td style="padding: 0 15px;padding-top:15px;padding-bottom:15px;"><b>Optional products:</b></td>'
	                    +'<td style="padding: 0 15px;padding-top:15px;padding-bottom:15px;">';
	                    <%for (int a = 0; a < Integer.parseInt(request.getParameter("OPSize"+i)); a += 1) {%>
	            		 <% String b = ("" + a).trim();%>
	            			command += '<%out.print(request.getParameter(i+"OP"+b));%>';
	            			<%if(a != Integer.parseInt(request.getParameter("OPSize"+i))-1){%>
	            				command += ', ';
	            			<%}%>
	            		 <%}%>
	            	  command += '</td></tr>'
	                  +'<tr style="padding:50px;">'
	                    +'<td style="padding: 0 15px;"><b>Price:</b></td>'
	                    +'<td style="padding: 0 15px;">' + "<%out.print(request.getParameter("PackageRPrice" + i));%>" + '</td>'
	                  +'</tr>'
	                +'</table>'
	                +'<br>'
	                +'<div class="d-flex justify-content-center">'
	                  +'<div class="d-flex justify-content-center">'
	                  +'<form method="POST" action="<%= request.getContextPath() %>/RetryPurchase.do">'
	                    +'<input type="hidden" name="PurchaseID" value="<%out.print(request.getParameter("PurchaseID" + i));%>">' 
	                    +'<input type="hidden" name="status" value="true">' 
	                    +'<button  style="margin-right: 20px;" class="btn btn-outline-success" type="submit" formmethod="post">Buy</button>'
	                    +'</form>'
	                    +'<form method="POST" action="<%= request.getContextPath() %>/RetryPurchase.do">'
	                    +'<input type="hidden" name="PurchaseID" value="<%out.print(request.getParameter("PurchaseID" + i));%>">' 
	                    +'<input type="hidden" name="status" value="false">' 
	                    +'<button style="margin-left: 20px;" class="btn btn-outline-danger" type="submit" formmethod="post">Buy</button>'
	                    +'</form>'
	                    +'</div>'
	              +'</div>'
	              +'</div>'
	              +'<div class="card-footer">'
	                +'<small class="text-muted"><b>Order ID: ' + "<%out.print(request.getParameter("PurchaseID" + i));%>" + '</b></small>'
	              +'</div></div></div>';
            <%}%>
            container.innerHTML += command;
  </script>
